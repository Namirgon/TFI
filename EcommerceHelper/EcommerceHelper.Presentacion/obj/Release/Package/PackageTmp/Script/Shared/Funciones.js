...
function onBtnAddClick(btn) {
    var control = $(btn);
    var idServicio = control.data('servicio');
    if (idServicio == null)
        idServicio = $('#IdServicioComprarServer').val();
    $.ajax({
        type: "POST",
        url: "MenuPrincipal.aspx/AgregarDeseo",
...
//END********Para quitar servicio de Lista Deseos*************END
...
//******Para colocar servicio de lista deseos en Pedidos al hacer click en Comprar en un produto Deseado*********
function onBtnComprar(btn2) {
    var control = $(btn2);
    var idServicioComprar = control.data('servicio2');

    consultarStockClickComprarEnDesear(idServicioComprar, function (stockActual) {
        if (stockActual > 0) {
            $.ajax({
                type: "POST",
                url: "/Areas/Public/Forms/ListaDeDeseos.aspx/ComprarProducto",
                data: '{ IdProdC: ' + idServicioComprar + '}',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                error: function (xhr, status, error) {
                    alert(error);
                ...
                success: function (result) {
                    ...
                    updateServicio();//Para cargar los servicios en el pedido al inicial la página
                    app.reload();
                }
            });
        }
...
//END******Para colocar servicio de lista deseos en Pedidos al hacer click en Comprar en un produto Deseado*********END


//Todos los class="btn-comprar" vienen primero aca
$('.btn-comprar').click(function () {
    var control = $(this);
    var idServicio = control.data('servicio');//Viene desde el .aspx propiedad: data-servicio
    if(idServicio == null)
        idServicio = $('#IdProductoComprarServer').val();
    //Llama a la función
    consultarStockClickComprar(idServicio, function (stockActual) {
        if (stockActual > 0) {
            $.ajax({
                type: "POST",
                url: "Pedidos.aspx/AgregarItem",//Ingresa en AgregarItem de Pedidos.aspx.cs y retorna el nombre del servicio
                data: '{ id: ' + idProducto + '}',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                error: function (xhr, status, error) {
                    //alert(error);
                    alert("Error al agregar servicio al Pedido, por favor reintente");
                },
                success: function (result) { //result es el nombre del servicio agregado
                    updateProductos();
